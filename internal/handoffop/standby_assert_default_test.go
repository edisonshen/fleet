...
// test lane in internal/handoffop. TestMain calls it LAST — after the
// socket-leak reap — replacing the trailing os.Exit(code), so the reap still
// runs before this exits. See internal/spawn/standby_counter.go for the shared
// counter and docs/DESIGN-spawn-test-fork-bomb-root-fix.md for the rationale.
