...
2. **Marker writer (single writer):** in `handoff_resume.py`, on successful `main()` (exit 0), write `resumed_handoff_path = <doc path>` via `coordinator.lock` with the same brief `LOCK_EX | LOCK_NB` retry used by the other coord-state writers. Prefer one locked RMW for the resumed-session stamp and marker so all `coord-state.json` writers converge on one mutex. Project resolution: `FLEET_PROJECT`, falling back to the handoff doc's frontmatter `project:` (always present) so an unset env can't silently no-op the write.
