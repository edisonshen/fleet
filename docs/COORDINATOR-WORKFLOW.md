...
- Scenario contract: one row per requirement the worker must prove on the built product — `| S | Requirement | Stand-up | Trigger | Observable outcome | Level | Harness |`. The observable outcome is what the operator would see (file content, pane text, exit code, PR state, TUI flash), never "function X is called". Level defaults to `e2e` (built binary + real tmux on an isolated `FLEET_TMUX_SOCKET` + real files under an isolated `FLEET_HOME`, via `internal/testutil/coorde2e`; see [`docs/TESTING.md`](TESTING.md)); `integ` is a package test at a real boundary (real FS, real flock, `tmuxfake`); `unit` is only for pure logic a scenario cannot reach and must carry its reason (`unit — <reason>`). Rows sharing a stand-up group into one table test, as the plan says. A task with nothing observable writes `none — <reason>` instead of a table.
...
- **Worker:** runs the scenario-first arc — `spec-repro` (reproduce every contract row on the built product in a sandbox, capture the wrong outcome verbatim), `spec-encode` (one test per row at its level, failing for that reason), `verify` (fix; re-run by hand and via the test; full gates exactly as `ci.yml`). Evidence goes in `~/.fleet/projects/<project>/workers/<slug>/verification.md` (`## Scenario contract`, `## Evidence — before`, `## Evidence — after`, `## Gates`, `## Baseline`, `## Unit tests`); a red gate that is the worker's own is fixed, never handed off, and a red believed pre-existing is re-run on untouched `origin/main` with both results under `## Baseline`. It records `--scenarios-total M --scenarios-verified N --gates-status passed`, commits locally, updates `phase=review-pending`, then exits. It does not run review and does not push.
- **Reviewer:** runs `/review` and `codex review` until clean with the Scenario-contract lens (every row has a test at its stated level asserting the observable outcome; `## Evidence — before` proves it would have failed; missing or downgraded rows are P1), fixes any P0/P1 findings with tests, and after any fix re-runs the touched scenario tests plus the full gates and appends `## Review re-verification` to verification.md. It records terminal review fields, updates `phase=review-done`, then exits. `/review` is never skippable; codex can be recorded as skipped only for the allowed reasons.
