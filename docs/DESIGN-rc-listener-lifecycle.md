...
|---|---|---|
| **(S1)** `skills/coordinator/remote_control.py:spawn_daemon_if_needed` | Coord tick respawns global `fleet-coord` daemon every 30s if absent | **Routes through controller** (codex round 2): the Python function shells out via `subprocess.run(["fleet", "rc", "up", project, "--respawn-only", "--idempotent"])` and returns based on outcome. `--respawn-only` (post-impl codex P1 catch) makes Go return `not_enabled` exit 0 when the marker is absent — the implicit-respawn path MUST never auto-create a marker, only the operator-explicit `fleet rc up <project>` does. Python no longer does its own pgrep/spawn — Go controller is the single owner. Per-project isolation via Claude daemon's dir-registry (working_dir field in rc-state.json), not prefix-filter renaming. |
