...
- Tests come with the feature, not after. CI (`.github/workflows/ci.yml`) runs `go build ./...`, `gofmt -l .`, `golangci-lint run ./...`, `go test -race ./...`, and `python3 -m pytest skills/ -q` on every PR and push to main (separate from the release-on-tag CI). Local lint: `golangci-lint run ./...` (config at `.golangci.yml`).
