...
- Lease fenced (`fleet lease-check` exit 3): skip the tick — no mutation, no
  ...
  (DESIGN-coord-lease-false-fence-prevention): the Go lease-check re-acquires
  a rival-free expired lease in place at the same epoch, so a fence means a
  rival takeover is live (or a transient) and the next tick re-checks.
  Session teardown of a genuinely superseded coord belongs to
  handoff/drain/gc, never to the fence path. The duplicate-coord lock-busy
  self-exit below is a SEPARATE route and stays.
