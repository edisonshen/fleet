...
On COORD sessions (explicit `FLEET_ROLE=coord`; the agent-record fallback is not honoured here) the Stop hook also captures the conversation into Key Decisions (`decisions.py`): it walks the transcript for the LAST operator turn (a human `user` entry or an `[OPERATOR]` inbox delivery — never tool results, `isMeta` entries, or the hook's own `HANDOFF REQUESTED` / `[FLEET]` injections) and pairs it with the coord's final assistant text after it, then shells `fleet checkpoint decision "operator: <prompt> → coord: <reply>"` (each half flattened and truncated to 240 chars). An exchange that looks like it carries a credential (known token prefixes — `ghp_`, `github_pat_`, `sk-`, `AKIA…`, `xox?-`, Stripe/Slack/Google/npm/HF shapes — a `PRIVATE KEY` block, a JWT, `Bearer …`, a `password=`/`api_key=`-style assignment, or a `user:pass@host` URL; `decisions.looks_secret`) is dropped whole, not redacted, checked on the untruncated text — coord-state.json outlives the transcript via handoff docs and tasks.md. A cursor at `~/.fleet/coord-decisions/<id>.cursor` holds the recorded turn's uuid so a blocked-and-continued turn records once (a vetoed turn also advances it, so the same secret isn't re-examined every Stop); a failed CLI write leaves the cursor alone and retries on the next Stop. This runs BEFORE the handoff trigger so a doc written on the same fire already carries the exchange.
