...
// returns. ENOENT on state.json is a hard error in the non-follow path
// (the operator passed a slug that doesn't exist).
func peekOnce(project, slug string, withLogs bool, stdout io.Writer) error {
	...
	if err != nil {
		if errors.Is(err, workers.ErrNotFound) {
			return fmt.Errorf("no worker %q in project %q (try `fleet workers list --project %s`)", slug, project, project)
		}
...
//   - state.json never appears within followTimeout (only at startup)
