...
func TestMaintenanceBootstrapReport_FlagsLiveAgentsMissingRC(t *testing.T) {
	...
	// (no action). The coord-spawn marker is gone (D3): the survey now
	// scopes to coords via spawn.IsCoordSpawn(TaskID, Project) — TaskID ==
	// "coord-<project>". All three records carry a coord TaskID so they
	// pass the coord filter; the exclusions below must therefore come from
	// the flag / dead-session filters, not the coord filter.
...
func TestMaintenanceBootstrapReport_StableOrderByOldestFirst(t *testing.T) {
	...
	// coord-spawn marker is gone (D3): the coord-only scope is task-based,
	// so each record carries a coord-<project> TaskID to pass the filter.
	records := []*agent.Record{
		{
			ID: "newer", Project: "p-newer", TaskID: "coord-p-newer", TmuxSession: "s-newer",
			SpawnedAt: t0.Add(2 * time.Hour),
			Command:   []string{"sh", "-c", `claude --dangerously-skip-permissions`},
		},
		{
			ID: "oldest", Project: "p-oldest", TaskID: "coord-p-oldest", TmuxSession: "s-oldest",
			SpawnedAt: t0,
			Command:   []string{"sh", "-c", `claude --dangerously-skip-permissions`},
		},
		{
			ID: "middle", Project: "p-middle", TaskID: "coord-p-middle", TmuxSession: "s-middle",
...
func TestMaintenanceBootstrapReport_SkipsNonCoordAgents(t *testing.T) {
	rcTestFleetHome(t)
	// The surveyed agent is a WORKER in a default-enabled project: its
	// TaskID is a worker slug, not coord-<project>, so spawn.IsCoordSpawn
	// (the coord-only scope after the D3 marker deletion) rejects it.
